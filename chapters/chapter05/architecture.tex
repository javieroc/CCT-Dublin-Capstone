...
The \texttt{scan} command accepts four main arguments:
\begin{itemize}
    ...
    \item \textbf{--output (-o):} An optional path to a file where the results will be saved in JSON format. If not provided, the results are printed to the standard output.
    \item \textbf{--verbose (-v):} An optional flag to enable verbose output. When enabled, the tool provides more detailed information about the scanning process, including which files are being scanned and the specific patterns being matched. This is useful for debugging and for a deeper understanding of the scan results.
\end{itemize}
